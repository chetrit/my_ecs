...
class registry_t {
  public:
    ...
    sparse_array<Component> &register_component() {
        ...
        _erase_component_funcs.push_back([](entity_t const &e, registry_t &r) { r.remove_component<Component>(e); });
    ...
    void add_system(Callable &&f) {
        _run_system_funcs.push_back(
            [f = std::forward<Callable>(f)](registry_t &r) { f(r, r.get_components<Components>()...); });
    };

    template <class... Components, typename Callable>
    void add_system(Callable const &f) {
        _run_system_funcs.push_back([&f](registry_t &r) { f(r, r.get_components<Components>()...); });
    }
  ...
  private:
    ...
    std::vector<std::function<void(entity_t const &entity, registry_t &registry)>> _erase_component_funcs;
